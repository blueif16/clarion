...
- **MiniMax is the brain + the voice.** **MiniMax-M3** (OpenAI-compatible, `api.minimax.io/v1`) is the kernel `Reasoner` (plan + per-step decide) and — via MiniMax's Anthropic-compatible gateway (LiveKit `anthropic` plugin; native `thinking` blocks dropped from spoken text) — the voice-plane LLM; **MiniMax-M2.7** is the failover (`llm.FallbackAdapter`, so an M3 5xx fails over instead of going silent). **MiniMax Speech 2.6-turbo** (`/v1/t2a_v2`) is the kernel `Synthesizer` + the LiveKit TTS. The LiveKit MiniMax TTS is wrapped by `_build_audio_tts`'s **one-segment adapter** (`_OneSegmentTTS`) — the 1.2.9 plugin opens an emitter segment per sentence, which agents 1.5.15 rejects mid-utterance (`RuntimeError: start_segment() called before the previous segment was ended`, crashed every multi-sentence reply); the wrapper collapses them into one segment. **Load-bearing — don't remove it** (live-verified 2026-06-06). `adapters/gemini_reasoner.py` / `openai_reasoner.py` are kept as alternate Reasoner backends behind the same guard.
