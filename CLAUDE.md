...
- **LiveKit** voice transport live · **Deepgram** STT live — now **Flux** (`STTv2`, `flux-general-en`), Deepgram's real-time-AGENT ASR (nova-3 is positioned for meetings/captioning) with **model-native phrase-endpointing turn detection** (`turn_detection="stt"`), which REPLACES the separate `MultilingualModel` EOU detector + the nova-3 `endpointing_ms` mid-word-finals hack. Built in `app/voice_entry._build_stt()`; **`STT_MODEL=nova-3` rolls back** to the prior `nova-3`+`smart_format`+EOU path. **EN-only by design** (`flux-general-en`; `STT_LANGUAGE`/`flux-general-multi`+`language_hint` for code-switch): Deepgram can't code-switch EN+Chinese in one stream — Chinese needs a dedicated `zh-CN`/`zh-HK` model. NB Flux has no `smart_format`.
