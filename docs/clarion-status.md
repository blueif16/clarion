...
_Last updated: 2026-06-07 · committed on branch **`feat/reasoner-rich-context`** (`989c2e8` rich context + specific plans + full tracing · `c31f4b1` schema trim) — not yet pushed/merged._
...
- _**212 green.** Follow-ups (not done): semantic top-K `ContextRanker` (shrink the `target_index` enum → faster constrained decode) + prompt-caching the static prefix; a full live extension run to capture end-to-end `decide_ms` (this capture is a synthetic-page micro-benchmark, not gov_proof/Moss)._
