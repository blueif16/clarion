...
_DECIDE_SYSTEM = (
    ...
    "PAGE MUST be copied verbatim from one of the grounded facts listed (via "
    "value_ref). Never invent or paraphrase a page value; if nothing grounded "
    "answers, leave 'say' empty. (The USER'S OWN input — a search term, a date or "
    "name they told you — is the exception: it has no page source, so type it via "
    "'fill_text', never value_ref.)\n"
...
_PLAN_SYSTEM = (
    ...
    "multiple steps; a site search is ONE subgoal — the search field is filled and "
    "submitted in a single step (Enter), so never plan a separate 'click the search "
    "button' subgoal. Each subgoal names a registered done_check from the allowed "
