...
Deterministic, network-free unit tests for ``kernel.negative_verifier`` +
``kernel.policy.is_negative_claim``. The killer acceptance (architecture migration
Step 5): a charge rendered as an IMAGE/canvas (invisible to the AXTree → no fact at
all) must HEDGE, not become a confident "no late fee."
  ...
  (5) ``is_negative_claim`` routes only asserted negatives through the verifier.
...
from clarion.kernel.policy import is_negative_claim
...
# ---------------------------------------------------------------------------
# (5) is_negative_claim — only asserted negatives route through the verifier
# ---------------------------------------------------------------------------


def test_is_negative_claim_detects_negatives_only() -> None:
    assert is_negative_claim("no late fee") is True
    assert is_negative_claim("There is no autopay enrolled") is True
    assert is_negative_claim("You don't have a balance") is True
    assert is_negative_claim("free of charge") is True
    # A positive read-back trips nothing (already fenced by membership).
    assert is_negative_claim("Amount due: $84.32") is False
    assert is_negative_claim("Confirmation #12345") is False
